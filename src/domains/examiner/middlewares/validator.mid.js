...
const validator = {
  ...
  evaluateProject: celebrate({
    ...
    body: Joi.object({
      ...
      premiumNomination: Joi.boolean().required(),
      score: Joi.object({
        relevancy: Joi.number().min(0).max(5).required(),
        originality: Joi.number().min(0).max(5).required(),
        methodology: Joi.number().min(0).max(5).required(),
        quality: Joi.number().min(0).max(5).required(),
        impact: Joi.number().min(0).max(5).required(),
      }),
